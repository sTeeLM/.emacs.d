...
(defun mew-mbox-buffer-find (case proto)
  "根据case和proto查找并返回buffer"
  (let ((buffer))
    (dolist (elm mew-mbox-buffer-alist)
      (when (and
             (string= (plist-get (cdr elm) 'case) case )
             (string= (plist-get (cdr elm) 'proto) proto ))
        (setq buffer (car elm))))
  buffer))

(defun mew-mbox-buffer-count-biff ()
  "累加所有buffer的biff-cnt并返回"
  (let ((cnt 0))
    (dolist (elm mew-mbox-buffer-alist)
      (setq cnt (+ cnt (plist-get (cdr elm) 'biff-cnt))))
    cnt))
